...
## Required parameters
flags.DEFINE_string(
        "data_dir", "/sdb/fffan/2_relevant_experiment/9_NLP/0_tensorflow_code/albert_zh-master/data/self_data",
        "The input data dir. Should contain the .tsv files (or other data files) "
        "for the task.")

flags.DEFINE_string(
        "bert_config_file", "./prev_trained_model/albert_tiny_489k/albert_config_tiny.json",
        "The config json file corresponding to the pre-trained BERT model. "
        "This specifies the model architecture.")

flags.DEFINE_string("task_name", "selfdata", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "./prev_trained_model/albert_tiny_489k/vocab.txt",
                                        "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
        "output_dir", "./outputs/selfdata_output",
        "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
        "init_checkpoint", "./prev_trained_model/albert_tiny_489k/albert_model.ckpt",
...
class SelfProcessor(DataProcessor):
    ...
    def get_labels(self,data_dir):
        """See base class."""
        if os.path.exists(os.path.join(data_dir, "label.txt")):
            labels = read_files(os.path.join(data_dir, "label.txt"))
        else:
            class_list = []
            ...
            print("Lables: ",labels)
            write_file(labels,os.path.join(data_dir, "label.txt"))
            #####     标签映射
            self.label_map = {}
            for i,one_label in enumerate(labels):
                self.label_map[one_label] = str(i)
        #####
        return labels
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(tqdm(lines)):
            ...
            text_b = None
            #label = tokenization.convert_to_unicode(line[1]) if set_type != 'test' else "100"
...
def main(_):
            ...
            for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
                ...
                for key in sorted(result.keys()):
                    if key == "eval_accuracy":
                        inference_result[global_step] = result[key]
                    tf.logging.info("    %s = %s", key, str(result[key]))
                    writer.write("%s = %s\n" % (key, str(result[key])))
        #######################################################################################################################
        inference_result_sort = sorted(inference_result.items(), key=lambda x: x[1], reverse=True)
    ...
    if FLAGS.do_eval:
        print("##############  Test   ########################################################################")
        eval_examples = processor.get_test_examples(FLAGS.data_dir)   ####  Test
        num_actual_eval_examples = len(eval_examples)
        ...
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
                ...
                tf.logging.info("***** Test results %s *****" % (filename))
                writer.write("***** Test results %s *****\n" % (filename))
                for key in sorted(result.keys()):
                    if key == "eval_accuracy":
                        inference_result_test[global_step] = result[key]
                    tf.logging.info("    %s = %s", key, str(result[key]))
                    writer.write("%s = %s\n" % (key, str(result[key])))
        #######################################################################################################################
        inference_result_sort_test = sorted(inference_result_test.items(), key=lambda x: x[1], reverse=True)

    print("############ eval result  #########################################")
    for step, acc in inference_result_sort:
        print("## Step :", step, "  eval_accuracy:", acc)
    print("################################################################")

    print("############ Test result  #########################################")
    for step, acc in inference_result_sort_test:
        print("## Step :", step, "  eval_accuracy:", acc)
    print("################################################################")
