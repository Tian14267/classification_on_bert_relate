...
def serving_input_fn():
	...
	class_num = 97
	seq_len = 128
	label_ids = tf.placeholder(tf.int32, [None, class_num], name='label_ids')
	input_ids = tf.placeholder(tf.int32, [None, seq_len], name='input_ids')
	input_mask = tf.placeholder(tf.int32, [None, seq_len], name='input_mask')
	segment_ids = tf.placeholder(tf.int32, [None, seq_len], name='segment_ids')
	...
	if FLAGS.do_eval:
		...
		with tf.gfile.GFile(output_eval_file, "w") as writer:
			for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
				...
				'''输出 pb 模型：trans_model_dir模型转换后输出目录,将模型转换为saved model'''
